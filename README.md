...
Three levels. **Summary strip** at the top: six tiles (Account, Drawdown, Concentration, Vs index, Peers, Plan), each a headline number, a one-line sub and a status pill; clicking a tile opens its section. **Sections** are collapsible `<details>` blocks, closed by default, each with a one-sentence summary line so the closed page reads as a report; open state resets on reload. **Detail toggles** inside sections hide the tables behind "+ …" links. Order: Drawdown → Rebalance Plan → Portfolio Review → Benchmark → Composition → Holdings. Each fact has one owner: cash lives in the Account tile and Drawdown, sector deviation in Benchmark Comparison, concentration stats in Benchmark and the Concentration tile.
